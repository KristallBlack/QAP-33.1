...
def test_forgot_password_page(browser):
    ...
    далее внутри этого теста рассматриваются 8 различныхт тестовых сценариев"""
    ...
    time.sleep(30)  # подождём, пока на почту поступит письмо
    ...
    error_mess = browser.find_element(*AuthLocators.AUTH_MESS_ERROR)
    assert error_mess.text == 'Пароль должен содержать хотя бы один спецсимвол или хотя бы одну цифру'
